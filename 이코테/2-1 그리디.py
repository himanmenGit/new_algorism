...
모험가 N명
N명의 모험가의 공포도 존재
공포도가 X인 모험가는 반드시 X명 이상 으로 구성한 모험가 그룹에 참여 가능
최대 몇 개의 모험가 그룹을 만들수 있는가
N명의 모험가에 대한 정보가 주어 졌을 때 여행을 떠날 수 있는 그룹 수의 최대값
"""
...
"""
내 풀이
내림차순으로 정렬하여 큰 공포도를 가진 모험가부터 채워 넣는다고 생각
틀림
이유: 해당 문제는 모든 모험가를 그룹에 포함하는것이 아니라 많은 그룹을 여행 보내는것
그렇다면 작은 공포도를 가진 모험가를 최대한 여행을 보내면 된다
왜냐하면 한 그룹에 여러명의 모험가를 넣는다고 그룹의 수가 늘어나지 않기 떄문
"""
# N = int(input())
